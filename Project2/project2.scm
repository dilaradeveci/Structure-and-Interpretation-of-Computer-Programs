...
;_BANNER_
;;;
;;; _EMAIL_ ddeveci18@ku.edu.tr    _DATE_ 11.11.2019
;;;
;;; Before you start:
;;;
...
(define (copy-first lst) (append lst (list(first lst))))

(define (create-polygon ptlist)
  (if (null?(cdr ptlist))
    '()
  (cons (make-seg (first ptlist) (second ptlist)) (create-polygon(cdr ptlist)))
  )
 )

; test:
(create-polygon (copy-first test-points))
...
(define (add-all-points lst)
  (if (null? lst) (make-point 0 0)   
      (add-two-points (first lst) (add-all-points(cdr lst)))
  )
)
...
(define (centroid lst)
  (make-point (/ (x-of(add-all-points lst)) (len lst)) (/ (y-of(add-all-points lst)) (len lst)))
)
...
(define add-x (lambda (lst)
  (if (null? lst) 0
      (+ (x-of (first lst)) (add-x (cdr lst))))
      ))

(define add-y (lambda (lst)
  (if (null? lst) 0
      (+ (y-of (first lst)) (add-y (cdr lst))))
      ))

(define (centroid-alt lst)
  (make-point (/ (add-x lst) (length lst)) (/ (add-y lst) (length lst)))
)
; test
...
;;Exercise 6 answer:
;;Since the square function takes a number as an input and cos is a procedure, Scheme will give error saying
;;"contract violation, expected: number?, given: #<procedure:cos>. Although it looks logical there is mismatch of types. 
...
;;Exercise 8 answer:
;;Since square-a-procedure takes a procedure which takes a number as input but
;;in deriv-squared there is only function but not number so Scheme gives error saying "arity mismatch,
;;the expected number of arguments does not match the given number, expected: 2,  given: 1"
...
(define (diagonal-line pt len)
    (lambda(t) (define delta (* t (/ len (sqrt 2))))
      (make-point (+ delta (x-of pt)) (+ delta (y-of pt)))
     )
  )
...
((diagonal-line p2 (* 5 (sqrt 2))) 0)  ;;(0 4)
((diagonal-line p3 (* 5 (sqrt 2))) 0)  ;;(2 5)
((diagonal-line p4 (* 5 (sqrt 2))) 0)  ;;(4 4)
...
(define (connect-ends curve1 curve2)
  (define delta-x (- (x-of (curve1 1)) (x-of (curve2 0))))
  (define delta-y (- (y-of (curve1 1)) (y-of (curve2 0))))
  (connect-rigidly curve1 ((translate delta-x delta-y) curve2))
)
...
;;Exercise 14 answer:
;;;unit-circle uses t as input so spaces between the points are steady, but the alternative-unit-circle takes t^2 as input
;;;so the spaces between points decrease as t^2 values becomes closer and closer for smaller numbers so the spaces are decreasing
;;;as the values get smaller instead of being steady.
...
(define (param-gosperize theta)
  (lambda (curve)
    (put-in-standard-position
     (connect-ends
      ((rotate-around-origin theta) ((scale (/ (/ 1 (cos theta)) 2)) curve)) ((rotate-around-origin (- theta)) ((scale (/ (/ 1 (cos theta)) 2)) curve))  
      ))))
...
((draw-connected g1 200)(param-gosper 6 (lambda(level) (/ pi (+ level 2)))))
((draw-connected g2 200)(param-gosper 6 (lambda(level) (/ pi (expt 1.3 level)))))
...
(define (kochize curve level)
  (define first ((scale (/ 1 4)) curve))
  (define second_1 ((rotate-around-origin (/ pi 3)) ((scale (/ 1 4)) curve)))
  (define second_2 ((rotate-around-origin (- (/ pi 3))) ((scale (/ 1 4)) curve)))
  (define third ((scale (/ 1 4)) curve))
  (if (= level 0)
      ((scale (/ 3 2)) curve)
      (kochize (connect-ends (connect-ends (connect-ends first second_1) second_2) third) (- level 1))
  ))
...
((draw-connected-squeezed-to-window g1 500)(kochize unit-line 1))
((draw-connected-squeezed-to-window g2 500)(kochize unit-line 2))
((draw-connected-squeezed-to-window g3 500)(kochize unit-line 3))
...
((draw-points-squeezed-to-window g2 10000)((scale 4)(connect-ends (param-gosper 20 (lambda(level) (/ (* 5 pi) level))) (gosper-curve 5))))
